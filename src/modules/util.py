...
class ResBlock3d(nn.Module):
    ...
    def __init__(self, in_features, out_features=None, kernel_size=None, padding=None):
        super(ResBlock3d, self).__init__()
        if out_features is None:
            out_features = in_features
        self.conv1 = nn.Conv3d(in_channels=in_features, out_channels=out_features, kernel_size=kernel_size, padding=padding)
        self.conv2 = nn.Conv3d(in_channels=out_features, out_channels=out_features, kernel_size=kernel_size, padding=padding)
        self.norm1 = nn.BatchNorm3d(in_features, affine=True)
        self.norm2 = nn.BatchNorm3d(out_features, affine=True)
...
class ResBlock2d(nn.Module):
    ...
    def __init__(self, in_features, out_features=None, kernel_size=None, padding=None, negative_slope=0.01):
        super(ResBlock2d, self).__init__()
        if out_features is None:
            out_features = in_features
        self.conv1 = nn.Conv2d(in_channels=in_features, out_channels=out_features, kernel_size=kernel_size, padding=padding)
        self.conv2 = nn.Conv2d(in_channels=out_features, out_channels=out_features, kernel_size=kernel_size, padding=padding)
...
class ResBlock3D_stage3_leak(nn.Module):
    def __init__(self, in_channels, out_channels, upsample=False, scale_factors=(1, 1, 1), negative_slope=0.01):
        super(ResBlock3D_stage3_leak, self).__init__()
        self.upsample = upsample
        self.scale_factors = scale_factors
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1)
        self.gn1 = nn.GroupNorm(num_groups=32, num_channels=out_channels)
