...
def concat_frames(driving_image_lst, source_image_lst, I_p_lst, driving_can_image_lst=None):
    ...
    for idx, _ in track(enumerate(I_p_lst), total=len(I_p_lst), description='Concatenating result...'):
        ...
        if driving_image_lst is None:
            ...
            if driving_can_image_lst is not None:
                driving_can_image = driving_can_image_lst[idx]
                driving_can_image_resized = cv2.resize(driving_can_image, (w, h))
                out = np.hstack((driving_image_resized, driving_can_image_resized, source_image_resized, I_p))
            else:
...
def to_frames(source_image_lst):
    # TODO: add more concat style
    out_lst = []
    h, w, _ = source_image_lst[0].shape
    source_image_resized_lst = [cv2.resize(img, (w, h)) for img in source_image_lst]  # Resize all source images

    for source_image_resized in source_image_resized_lst:
        out = source_image_resized  # In this case, just append the resized source image
        out_lst.append(out)
