...
def dilation_mask(mask, kernel_size=5, device="cuda"):
    mask = mask  # (H, W) -> (1, 1, H, W)
    kernel = torch.ones((1, 1, kernel_size, kernel_size), device=device)
    mask = F.conv2d(mask, kernel, padding=kernel_size // 2)
    return (mask > 0).float()

def erode_mask(mask, kernel_size=5, device='cuda'):
    mask = mask.float()
    mask = mask  # (H, W) -> (1, 1, H, W)
    mask = -F.max_pool2d(-mask, kernel_size, stride=1, padding=kernel_size // 2)  # 取最小值
    return (mask > 0).float()

def smooth_mask(mask, kernel_size=5, device="cuda"):
    mask = mask
...
def crop_image_mo2c(img, pts: np.ndarray, mo2c, **kwargs):
    dsize = kwargs.get('dsize', 224)
...
def crop_double_image(tar_img, tar_pts: np.ndarray, can_img, can_pts: np.ndarray, **kwargs):
    #按照target的crop方式同时对target和canonical进行crop和align
    dsize = kwargs.get('dsize', 224)
    scale = kwargs.get('scale', 1.5)  # 1.5 | 1.6
    vy_ratio = kwargs.get('vy_ratio', -0.1)  # -0.0625 | -0.1
    ...
    tar_img_crop = _transform_img(tar_img, M_INV, dsize)  # origin to crop
    can_img_crop = _transform_img(can_img, M_INV, dsize)  # origin to crop
    tar_pt_crop = _transform_pts(tar_pts, M_INV)
    can_pt_crop = _transform_pts(can_pts, M_INV)
    ...
    ret_dct = {
        'M_o2c': M_o2c,  # from the original image to the cropped image 3x3
        'M_c2o': M_c2o,  # from the cropped image to the original image 3x3
        'tar_img_crop': tar_img_crop,  # the cropped image
        'tar_pt_crop': tar_pt_crop,  # the landmarks of the cropped image
        'can_img_crop': can_img_crop,  # the cropped image
        'can_pt_crop': can_pt_crop,  # the landmarks of the cropped image
    }
...
def prepare_paste_back(mask_crop, crop_M_c2o, dsize, if_float=False):
    """prepare mask for later image paste back
    """
    mask_ori = _transform_img(mask_crop, crop_M_c2o, dsize)
    if if_float is False:
