...
class Cropper(object):
    ...
    def crop_source_image(self, img_rgb_: np.ndarray, crop_cfg: CropConfig, lmk = None):
        ...
        if lmk is None:
            if self.image_type == "human_face":
                ...
                # 检查人脸宽度，如果小于 120 则返回 None
                # 假设 src_face.bbox 格式为 [x1, y1, x2, y2]
                # print(src_face.bbox)
                # exit()
                # face_width = src_face.bbox[2] - src_face.bbox[0]
                # if face_width < 125:
                #     log(f"Detected face width {face_width} is less than 120. Skipping this image.")
                #     return None
    ...
    def crop_video_with_mo2c(self, source_rgb_lst, crop_cfg: CropConfig, mo2c, **kwargs):
        """Tracking based landmarks/alignment and cropping"""
        trajectory = Trajectory()
        direction = kwargs.get("direction", "large-small")
        for idx, frame_rgb in enumerate(source_rgb_lst):
