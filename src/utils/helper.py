...
def add_ddp_prefix(state_dict):
    """
    Add the 'module.' prefix to the keys in a state_dict.

    Args:
        state_dict (dict): The original state_dict without DDP prefixes.

    Returns:
        OrderedDict: A new state_dict with 'module.' prefixed to each key.
    """
    state_dict_new = OrderedDict()
    for key in state_dict.keys():
        state_dict_new[f"module.{key}"] = state_dict[key]
    return state_dict_new
...
def load_model(ckpt_path, model_config, device, model_type):
    ...
    # if model_type == 'spade_generator':
    #     old_weight = model_state_dict['conv_img.0.weight']  # 形状 [12, out_channels, 3, 3]
    #     new_weight = old_weight[[1,5,9]]                  # => [3, out_channels, 3, 3]
        
    #     # 把新得到的权重放回字典，并命名为 "conv_img.weight"
    #     model_state_dict['conv_img.weight'] = new_weight
    #     # 删除旧 key，避免冲突
    #     model_state_dict.pop('conv_img.0.weight')

    #     old_bias = model_state_dict['conv_img.0.bias']  # 形状 [12]
    #     # old_bias = old_bias.view(4, 3).mean(dim=0)      # => [3]
    #     model_state_dict['conv_img.bias'] = old_bias[[1,5,9]]
    #     model_state_dict.pop('conv_img.0.bias')
    model.load_state_dict(model_state_dict)
    # state_dict = torch.load(ckpt_path, map_location=lambda storage, loc: storage)
    # print(state_dict.conv_img.0.weight.shape, state_dict.conv_img.0.bias.shape)
