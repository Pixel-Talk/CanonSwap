...
class ArgumentConfig(PrintableConfig):
    ...
    driving:  Annotated[str, tyro.conf.arg(aliases=["-t"])] = make_abs_path('../../assets/examples/driving/d0.mp4')  # path to target video or template (.pkl format)
    output_dir: Annotated[str, tyro.conf.arg(aliases=["-o"])] = 'results/'  # directory to save output video
    ...
    flag_crop_driving_video: bool = True  # whether to crop the driving video, if the given driving info is a video
    ...
    source_max_dim: int = 4096 # the max dim of height and width of source image or video, you can change it to a larger number, e.g., 1920
